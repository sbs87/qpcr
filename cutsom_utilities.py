...
#Function for calculating mean given a vector of numerical values
def calc_mean(vals):
    sum=0
    count=0
    for val in vals:
...
#Function to calculate standard deviation given a vector of numerical values. Depends on calc_mean function above
def calc_stdev(vals):
    m=calc_mean(vals)
    sum=0
    count=0
...
#Returns a vector of unique values given a vector of redundant values
def unique(vals):
    seen=[]
    for val in vals:
        if val not in seen:
